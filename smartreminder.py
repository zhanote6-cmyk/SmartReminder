...
# ─── НАСТРОЙКИ ───────────────────────────────────────────────────────────────
BOT_TOKEN = "8666538542:AAHivmbSlTWIIP9_pnd6YKKQv1N2LNa7GU8"   # ← Өз токеніңді қой
BOT_PASSWORD = "smart2026"
DATA_FILE = "user_data.json"

# index.html файлы жүктелген URL (GitHub Pages, Netlify, немесе басқа хостинг)
# Мысалы: "https://yourusername.github.io/smart-reminder/index.html"
WEBAPP_URL = "https://github.com/zhanote6-cmyk/SmartReminder"   # ← МІНДЕТТі: осыны өзгерт!
...
# ─── WEBAPP DATA HANDLER (шагомер app-тан деректі қабылдау) ──────────────────
async def webapp_data_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """index.html ішіндегі tg.sendData() осы функцияға түседі"""
    user_id = str(update.effective_user.id)
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "menu_reminders":
        ...
        text = "📅 *Апта жоспары*\n\n"
        for day in days:
            day_reminders = [r for r in reminders if r.get("day") == day]
            text += f"*{day}:*\n"
            if day_reminders:
            ...
            text += "\n"
        keyboard = [
            [InlineKeyboardButton("➕ Жоспар қосу", callback_data="reminder_add")],
            [InlineKeyboardButton("🗑 Барлығын тазалау", callback_data="reminder_clear")],
        ...
        days = ["Дүйсенбі", "Сейсенбі", "Сәрсенбі", "Бейсенбі", "Жұма", "Сенбі", "Жексенбі"]
        keyboard = [[InlineKeyboardButton(day, callback_data=f"rday_{day}")] for day in days]
        keyboard.append([InlineKeyboardButton("🔙 Артқа", callback_data="menu_reminders")])
        await query.edit_message_text("📅 Қай күнге жоспар қосасыз?", reply_markup=InlineKeyboardMarkup(keyboard))

    elif data.startswith("rday_"):
        day = data[5:]
        context.user_data["reminder_day"] = day
        await query.edit_message_text(
            f"*{day}* күніне жоспар қосу\n\nУақыт пен тапсырманы жіберіңіз:\nМысалы: `09:00 Дәрі ішу`",
            parse_mode="Markdown"
        )
        context.user_data["state"] = "adding_reminder"

    elif data == "reminder_clear":
        user = get_user(user_id)
        user["reminders"] = []
        update_user(user_id, user)
        await query.edit_message_text(
...
def main():
    ...
    # ⬇️ WebApp деректерін қабылдайтын handler (ең маңызды!)
